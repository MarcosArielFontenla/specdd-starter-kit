...
export function renderRolePrompt(role, cmd) {
  ...
  return `---
agent: agent
description: ${ROLE_META[role].title} command ${cmd} (SpecDD Harness capability pack)
---
...
export function capabilityManifestPath(role) {
  const slug = roleSlug(role);
  return `.agents/capabilities/${slug}/capability.json`;
}

export function capabilityForRole(role, input) {
  const meta = ROLE_META[role];
  const pack = createCapabilityPackFromRoleDescriptor({
    role: {
      id: role.toLowerCase(),
      title: meta.title,
      scope: meta.scope,
    ...
    selectedPlaybooks: input.skillsByRole?.[role] || [],
    commands: commandsFor(role, input),
  });
  const validation = validateCapabilityPack(pack);
  if (!validation.valid) {
    const details = validation.diagnostics.map((diagnostic) => `${diagnostic.path}: ${diagnostic.message}`).join('; ');
    throw new Error(`Invalid Capability Pack for ${role}: ${details}`);
  }
  return pack;
}

function assertCapabilityArtifacts(pack, files) {
  const references = [
    ...pack.skills,
    ...pack.playbooks,
    ...pack.workflows,
  ...
  const missing = references.map((entry) => entry.path).filter((path) => !(path in files));
  if (missing.length > 0) {
    throw new Error(`Capability Pack '${pack.metadata.id}' references artifacts that were not generated: ${missing.join(', ')}`);
  }
}
...
export function renderInstallTasks(input, today) {
  ...
  const rows = (input.roles || []).flatMap((role) => {
    const pack = capabilityForRole(role, input);
    const route = pack.routing[0];
    const skill = pack.skills.find((entry) => entry.id === route.skillRef);
    const manifestPath = capabilityManifestPath(role);
    return [
      `- [ ] R${pad3(++i)} Validate \`${manifestPath}\` against the Capability Pack 1.0.0 contract before wiring it`,
      `- [ ] R${pad3(++i)} Add ROUTING row: \`| ${route.match} | ${skill.path} | role workflows in ${route.workflowRoot} |\``,
      `- [ ] R${pad3(++i)} Add REGISTRY entries for \`${manifestPath}\`, its skills, evals, workflows and inactive subagent`,
      `- [ ] R${pad3(++i)} If \`context/project-definition.json\` exists, add capability binding \`{ "id": "${pack.metadata.id}", "source": "${manifestPath}", "version": "${pack.metadata.version}", "enabled": true }\` and validate the Project Definition`,
      `- [ ] R${pad3(++i)} Add budget-manifest class '${route.match}' with artifacts [.agents/orchestration/ROUTING.md, ${skill.path}]`,
    ];
  });
  return `---
...
# Tasks — Capability Pack Install

Pre-generated by the SpecForge wizard. A human must approve this file BEFORE the
agent wires anything. Capability manifests are declarative and never modify existing
harness files by themselves. Projects without a Project Definition retain the legacy
ROUTING/REGISTRY/budget installation path.
...
Each selected role is an independent Capability Pack under
\`.agents/capabilities/<role-id>/capability.json\`. Existing Role Pack paths remain
...
export function generatePack(baseSkills, input, today = new Date().toISOString().slice(0, 10)) {
  ...
  for (const role of roles) {
    ...
    const capability = capabilityForRole(role, input);
    files[capabilityManifestPath(role)] = `${JSON.stringify(capability, null, 2)}\n`;
    ...
    assertCapabilityArtifacts(capability, files);
  }
