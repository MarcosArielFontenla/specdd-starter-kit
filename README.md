...
8. For placeholder entity contracts, the agent prepares explicit candidate JSON files
   and runs `converge-contracts.ps1 -Mode propose`; review its exact subject before a
   separate `-Mode apply -SubjectSha256 <hash> -ReviewedBy <identity>` operation.
9. If approved implementation changes a fingerprinted source file, run
...
| [`packages/artifact-model`](packages/artifact-model/) | `@specdd/artifact-model` — versioned role artifacts, traceability graph, [BA domain](packages/artifact-model/BA.md), [QA domain](packages/artifact-model/QA.md) and [governed SpecDD projection](packages/artifact-model/PROJECTION.md). |
| [`packages/specforge-workspace`](packages/specforge-workspace/) | Accepted local BA Workspace MVP and Phase 5 projection: SQLite history, explicit real-agent proposals, exact human approval, reviewed diff/mapping and persistent local canonical specs. Runtime remains opt-in; no filesystem/Git publisher. |
