...
import { assertPlan, fingerprint, compareBenchmark } from '@specdd/benchmarks';
...
const fixtures = [fileURLToPath(new URL('./worker.test.mjs', import.meta.url))];
const implementation = fileURLToPath(import.meta.resolve('@specdd/eval-adapters'));
const pinnedPaths = [...fixtures, implementation, ...['baseline/index.js', 'candidate/index.js', '../schema/proposal.schema.json', '../examples/serial-profile.proposal.json', '../../benchmarks/examples/local/plan.json', '../../benchmarks/examples/local/dataset.json'].map(p => fileURLToPath(new URL(p, import.meta.url)))];
...
const profiles = await Promise.all(['baseline', 'candidate'].map(async id => ({ id, flags: [], executionRef: 'sha256:' + fingerprint({ node: process.version, flags: ['--test'], profile: id, implementation: createHash('sha256').update(await readFile(new URL('./' + id + '/index.js', import.meta.url))).digest('hex'), taskSha256: sha256 }) })));
const plan = { kind: 'SpecDDBenchmark', schemaVersion: '1.0.0', id: 'proposal-summary-reuse', version: '1.0.0', scope: 'eval-node',
  task: { id: 'assessment-equivalence-batch', inputRef: 'sha256:' + sha256, sha256 }, workflowRef: 'manual-review', repetitions: 5, baselineRef: 'baseline',
  eval: { kind: 'SpecDDEval', schemaVersion: '1.0.0', id: 'score-conformance', version: '1.0.0', name: 'Pinned score fixture conformance', method: 'mechanical',
    rubric: 'Run 30 assessments and require exact baseline output and input preservation. Exit zero passes; nonzero fails. Latency includes process startup.',
    labels: [{ id: 'passed', description: 'Assessment equivalence batch passed', score: 1 }, { id: 'failed', description: 'Assessment equivalence batch failed', score: 0 }], passingScore: 1 },
...
await save('environment.json', { ...environment, profiles, taskFiles: pinnedPaths.map(p => p.replaceAll('\\', '/').split('/').slice(-3).join('/')), warmup: 'none', order: 'alternating per repetition', limits: 'Inherited environment and system load not fully captured' });
const samples = [];
...
for (let repetition = 1; repetition <= plan.repetitions; repetition++) {
  for (const profile of repetition % 2 ? profiles : [...profiles].reverse()) {
    ...
    const child = spawnSync(process.execPath, ['--test', ...profile.flags, ...fixtures], { env: { ...env, PILOT_PROFILE: profile.id }, shell: false, windowsHide: true, encoding: 'utf8', timeout: 30000, maxBuffer: 1024 * 1024 });
