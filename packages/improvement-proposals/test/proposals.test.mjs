...
import { assertProposal, analyzeHistory, assessProposal, reviewState, preparePullRequest } from '../dist/index.js';
...
test('real CLI replay is read-only and exits 2 for unqualified evidence', () => {
  const files = ['../examples/serial-profile.proposal.json', '../../benchmarks/examples/local/plan.json', '../../benchmarks/examples/local/dataset.json', '../../benchmarks/examples/local/plan.json', '../../benchmarks/examples/local/dataset.json', '../examples/journal.json'].map(p => new URL(p, import.meta.url));
  const before = files.map(p => readFileSync(p));
  const result = spawnSync(process.execPath, [fileURLToPath(new URL('../scripts/inspect.mjs', import.meta.url)), ...files.map(p => fileURLToPath(p))], { encoding: 'utf8', windowsHide: true, timeout: 30000 });
