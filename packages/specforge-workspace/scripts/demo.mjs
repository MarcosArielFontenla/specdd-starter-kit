...
export async function demoBundle() {
  ...
  const skills=Object.fromEntries(await Promise.all(ROLE_SKILLS.BA.map(async name=>[name,await readFile(new URL(`../../../specforge-kit/skills/${name}.md`,import.meta.url),'utf8')])));
  const {files}=generatePack(skills,{roles:['BA'],tools:['Codex'],skillsByRole:{BA:ROLE_SKILLS.BA}},'2026-09-07');
  const source='.agents/capabilities/role-ba/capability.json',pack=JSON.parse(files[source]);
  project.capabilities=[{id:pack.metadata.id,source,version:pack.metadata.version,enabled:true}];
  ...
  return {project,capability:{pack,files,dependencies:[{id:'specdd-harness',kind:'harness',versionRange:'^1.0.0'}]}};
}
